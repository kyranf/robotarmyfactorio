...
function updateSquad(squad)
    ...
    if squad.unitGroup and squad.unitGroup.valid then  --important for basically every AI command/routine
        ...
        if squad.command == commands.guard then
            ...
            checkBattleAI(squad, getSquadHuntSize(squad.force)) --remove checks in this function for validity and possibly command
        end
...
function orderSquadToRetreat(squad)
    ...
    -- player.print(string.format("squad %d (size %d): valid? %s group_state? %s squad_command? %s", squad.squadID, squad.members.size,
    --                            tostring(squad.unitGroup.valid), squad.unitGroup.state,
    --                            squad.command))
    if squad.unitGroup.valid and squad.command == commands.hunt then
        ...
        if squad.members.size > 0 then
            entity, distance = global_findClosestForceAssemblerToPosition(squad.unitGroup.position, squad.force.name)
            ...
            if entity and distance > 10 then
                ...
                -- player.print(string.format("Nearest retreat location %s, at distance %d, is too close to squad %d for retreat.", entity, distance, squad.squadID))
            end
        else
            -- player.print(string.format("Can't retreat squad %d of size 0!!", squad.squadID))
        end
    else
        -- player.print(string.format("something is wrong with squad %d of size %d", squad.squadID, squad.members.size))
    end
end
...
function checkBattleAI(squad, squadSize)
    ...
    if squad.unitGroup then
        if (squad.unitGroup.valid
                and
                (squad.unitGroup.state == defines.group_state.gathering
